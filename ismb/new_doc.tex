...
regulation. 
\update{A recent review of the methods is available in \cite{Androulakis2007}.}
...
values. 
\update{HMMs have been previously used to analyze temporal
gene expression data~\cite{citeulike:5209494,citeulike:1069055,Angelini2008BATS}. }
...
network topology of regulation. 

\update{ We pose the problem of finding the time-varying interaction
  strengths as an inference over the HMM where the hidden state is the
  joint state of the interactions strengths over the known network
  topology. This has an exponentially large state space and can't be
  directly solved. We use the factorial assumption that the weights
  evolve independently to obtain a tractable approximation to the
  original problem.  [THIS WOULD
  ALREADY HAVE BEEN DONE - COMPARE VS SCHLIEP03, NAM09, KELLER, etc.]
}

\update{
Often, expression levels are present for multiple strains which are
  slightly perturbed versions of the original strain, in the sense
  that a few genes are knocked out of the original strain. Most
  existing methods [REFS NEEDED] solve the inference problem over
  separate strains independently. One of the
  contributions of this article is that we present a simple damping model
  which allows us to systematically include the information across
  several slightly perturbed strains in the inference algorithm.   
}

\update{There has been considerable effort in establishing a
  hierarchy of genes based on their functionality
  \cite{Bader:2003:Nucleic-Acids-Res:12519993,MIPS,citeulike:814974,citeulike:226627,citeulike:3733950}. The second contribution of this
  article is a novel method for incorporating the functional
  classifications in the analysis of temporal expression data. We
  model the evolution of interaction strength for a gene pair in terms
  of the evolution characteristics of the functional categories. This
  leads to the inference algorithm, NETGEM, which models the gene
...
catogories of the interacting genes during the inference
procedure. 
}
