function [G, H, FB, S] = netgem(graphData, exprData, sig_genes, ...
                                sparsityFactor, W, fbIters)
% NETGEM runs the experiment on the second dataset ( REF/MUT ) with 6 time
...
% 
% Usage: [G, H, FB, S] = netgem(graphData, exprData, sig_genes, sparsityFactor, W)
%
% Expects:
% ------------
...
%                   * exprData.expr_genes
%
%   sig_genes:  Subset of genes of interest
% 
%   sparsityFactor: Algorithm parameter controlling the variance in
%                   prior Q for functional categories 
%                   (default: 1.0)
% 
%   W:          Set of possible edge weights 
%               (default: [-2, -1, 0, 1, 2])
% 
%   fbIters:    Number of forward backward iterations (default: 15)
% 
% Returns:
% ------------
...
% 
    ...
    fprintf(2, '\n**********************************************************************************************');
    fprintf(2, '* exp2(): sparsityFactor: %g', sparsityFactor);
    fprintf(2, '**********************************************************************************************\n');
    
    %%% load the dataset
    load cats.mat;
    %%% The graph structure variables
    % load graph_pp.mat; 
    gene_graph = graphData.gene_graph;
    gene_names = graphData.gene_names; 
    gene_adj  = graphData.gene_adj; 
    
    %%% The gene expression data from the exprData structure
    % load dataExp2.mat
    expr_cond = exprData.expr_cond;
    expr_data = exprData.expr_data;
    expr_genes = exprData.expr_genes; 
    
    %%% data corresponding to experiment 2 data --- will change for
    %%% other datasets
    deleted = ['YLR403W'];
    nT = 6; 
    nS = 2; 
   ...
   % sig_genes = gene_names; 
    %%% find sub graph for sig_genes ---- allows selection of subset if
    %%% none specified then defaults to the complete set. 
    ...
    fprintf(2, 'netgem(): Pre-Inference: allocating X\n'); 
    X =zeros(nV, nT, nS); 
    X(:, :, 1) = c2_data(:, 1:nT); 
    ...
    fprintf(2, 'netgem(): Pre-Inference: allocating mixture components\n'); 
    c2_E = [I J];
