function [result] = exp3x2(tLowPct, tHighPct, W, N, nH, T)
...
%% Last-Updated: Mon Sep  6 17:01:01 2010 (+0530)
%%           By: Vinay Jethava
%%     Update #: 67
%% URL:
...
%%% Code:
    ...
    [cres, legends] = inner3x2_1x0(synthRes.trE, synthRes.sE, 1, ...
                                 synthRes, thresholds, markers, np1, ...
                                 np2);
    % hold on;
    ...
    legend(legends, 'Location', 'SouthEast');
    saveas(h, 'exp3x2fig2a.fig');
    print('-dpng', 'exp3x2fig2a');
    close(h); 
end

function [res, legends]  = inner3x2_1x0(t3, s3, count, synthRes, thresholds, ...
                                         markers, np1, np2 )
    legends = {};     
